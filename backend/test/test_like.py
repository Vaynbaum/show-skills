import pytest
from httpx import AsyncClient

from main import app
from test.common import *
from test.data.like_data import *
from test.data.user_auth_data import *

pytest_plugins = ("pytest_asyncio",)

# ----------------------Add like----------------------
@pytest.mark.asyncio
async def test_add_like_to_no_exist_post():
    headers = await get_header(USER_TEST_AUTH)
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.put(f"/like/?post_key={NO_EXIST_POST_KEY}", headers=headers)
    assert response.status_code == 404
    result = response.json()
    assert "detail" in result
    assert result["detail"] == "Post not found"


@pytest.mark.asyncio
async def test_add_remove_like_to_post():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        await ac.post("/auth/signup", json=SOBOLEV_REGISTRATION_VALID_DATA)
        headers = await get_header(SOBOLEV_AUTH_VALID_DATA)

        response = await ac.post("/post/create", json=POST_DATA, headers=headers)
        post_key = response.json()["key"]

        response = await ac.put(f"/like/?post_key={post_key}", headers=headers)

    assert response.status_code == 200
    result = response.json()
    assert "message" in result
    assert result["message"] == "The like has been set successfully"

    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.delete(f"/like/?post_key={post_key}", headers=headers)

    assert response.status_code == 200
    result = response.json()
    assert len(result) == 0

    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.delete(f"/post/?post_key={post_key}", headers=headers)
    await delete_auth(SOBOLEV_REGISTRATION_VALID_DATA)


# ----------------------Delete like----------------------
@pytest.mark.asyncio
async def test_delete_no_exist_like():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        await ac.post("/auth/signup", json=SOBOLEV_REGISTRATION_VALID_DATA)
        headers = await get_header(SOBOLEV_AUTH_VALID_DATA)

        response = await ac.post("/post/create", json=POST_DATA, headers=headers)
        post_key = response.json()["key"]

        response = await ac.delete(f"/like/?post_key={post_key}", headers=headers)

    assert response.status_code == 404
    result = response.json()
    assert "detail" in result
    assert result["detail"] == "Like not found"

    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.delete(f"/post/?post_key={post_key}", headers=headers)
    await delete_auth(SOBOLEV_REGISTRATION_VALID_DATA)


@pytest.mark.asyncio
async def test_delete_no_exist_like_to_no_exist_post():
    headers = await get_header(USER_TEST_AUTH)
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.delete(f"/like/?post_key={NO_EXIST_POST_KEY}", headers=headers)

    assert response.status_code == 404
    result = response.json()
    assert "detail" in result
    assert result["detail"] == "Post not found"

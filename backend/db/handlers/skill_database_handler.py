...
class SkillDatabaseHandler:
    ...
    async def create(self, skill: SkillCreateDataModel) -> Union[SkillModelInDB, None]:
    ...
    async def get_many_by_query(
        ...
        """
        result = await self.__skills_db.fetch(query, limit, last_skill_key)
        return ResponseItems[SkillModelInDB](
            count=result.count, last=result.last, items=result.items
        )

    async def get_by_key(self, key: str) -> Union[SkillModelInDB, None]:
